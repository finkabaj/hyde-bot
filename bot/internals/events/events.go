...
func (em *EventManager) HandleEvent(s *discordgo.Session, event interface{}) {
    ...
    castedEvent, ok := event.(*discordgo.MessageCreate)
	
		if !ok {
			logger.Error(errors.New("Error getting the guild ID on message create event"), "Error getting the guild ID on message create event")
			return
		}
 
		guildID := castedEvent.GuildID
