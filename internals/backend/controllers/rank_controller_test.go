...
func TestDeleteRank(t *testing.T) {
	data := []struct {
	...
	}{
		{
			...
			inputGuildID:       "1",
			inputRankID:        "1",
			expectedOkResponse: common.OkResponse{Message: "successfully deleted rank"},
			expectedStatus:     http.StatusOK,
			expectedError:      common.ErrorResponse{},
			rawError:           nil,
		},
		{
			...
			expectedOkResponse: common.OkResponse{},
			expectedStatus:     http.StatusNotFound,
			expectedError:      *common.NewErrorResponseBuilder(common.ErrNotFound).SetStatus(http.StatusNotFound).SetMessage("guild id or rank id not found").Get(),
			rawError:           common.ErrNotFound,
		},
		{
			name:               "Internal",
	...
	for _, d := range data {
		t.Run(d.name, func(t *testing.T) {
			mockRankService.On("DeleteRank", d.inputGuildID, d.inputRankID).Return(d.rawError)

			rr := httptest.NewRecorder()
			req := httptest.NewRequest("DELETE", fmt.Sprintf("/rank/%s/%s", d.inputGuildID, d.inputRankID), nil)
			r.ServeHTTP(rr, req)
			defer rr.Result().Body.Close()
